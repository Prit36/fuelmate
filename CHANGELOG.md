...
The current version is tracked in the `VERSION` file. To record a new release,
run `scripts\bump-version.bat` (see that script for usage). Each bump updates
this changelog, the `VERSION` file, and the Android `versionCode`/`versionName`
in `app/build.gradle.kts`, then commits and tags the release.
